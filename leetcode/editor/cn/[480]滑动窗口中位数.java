...
class Solution {
    ...
    public double[] medianSlidingWindow(int[] nums, int k) {
        ...
        ret[0] = countMid(Arrays.copyOf(nums, k));
        for (int i = 0, j = k; j < n; ++i, ++j) {
            int mid = (int) countMid(new int[]{nums[i], nums[j], (int) ret[i]});
            if (ret[i] == mid || k % 2 == 0) {
                ret[i + 1] = countMid(Arrays.copyOfRange(nums, i + 1, j + 1));
                continue;
            }
            ret[i + 1] = ret[i];
        }
    ...
    private double countMid(int[] nums) {
