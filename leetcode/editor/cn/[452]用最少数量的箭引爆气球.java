...
class Solution {
    public int findMinArrowShots(int[][] points) {
        ...
        Arrays.sort(sortedPoints, (a, b) -> Integer.compare(a[0], b[0]));
        int[] prePoint = sortedPoints[0].clone();
        int result = 1;
        for (int i = 1; i < sortedPoints.length; ++i) {
            if (prePoint[1] < sortedPoints[i][0]) {
                ++result;
                prePoint = sortedPoints[i].clone();
                continue;
            }
            prePoint[0] = sortedPoints[i][0];
            prePoint[1] = Integer.min(prePoint[1], sortedPoints[i][1]);
        }
        return result;
    }
}
